...
/*
  ...
  Description: Makes common changes to the Genesis Breadcrumbs easy as typing your name. Requires Genesis 1.6+
 */
...
if ( !defined( 'ABSPATH' ) ) {
    wp_die( __( "Sorry, you are not allowed to access this page directly.", GSB_TEXTDOMAIN ) );
}

define( 'GSB_PLUGIN_DIR', dirname( __FILE__ ) );
define( 'GSB_TEXTDOMAIN', 'GSB' );
...
function gsb_activation_check() {
    ...
    if ( basename( TEMPLATEPATH ) != 'genesis' ) {
        ...
        wp_die( sprintf( __( 'Sorry, you can\'t activate unless you have installed %1$sGenesis%2$s', GSB_TEXTDOMAIN ), '<a href="http://designsbynickthegeek.com/go/genesis">', '</a>' ) );
    }
    ...
    if ( version_compare( $version, $latest, '<' ) ) {
        deactivate_plugins( plugin_basename( __FILE__ ) ); // Deactivate ourself
        wp_die( sprintf( __( 'Sorry, you can\'t activate without %1$sGenesis %2$s%3$s or greater', GSB_TEXTDOMAIN ), '<a href="http://designsbynickthegeek.com/go/genesis">', $latest, '</a>' ) );
    }
}
